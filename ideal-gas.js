(function () {
  ...
  let coreInteraction = false;
  const showcaseHint = (message) => coreInteraction ? "继续改变体积，对照 pV=nRT 和图表中的当前点" : message;
  const markCoreInteraction = () => { coreInteraction = true; document.body.classList.add("has-core-interaction"); };
  const modes = {
    microscopic: { title: "微观压强", goal: "粒子碰壁的微观图像如何对应宏观压强？", hint: "先拖动活塞改变体积，观察粒子密度、压强和 p–V 图如何同步变化", badge: "热平衡", badgeClass: "balanced", key: "◎ 300 K 基准" },
  ...
  function collisionRatios(current) {
    const thermalRatio = Math.sqrt(current.temperature / Math.max(1e-9, state.baseTemperature));
    const volumeRatio = current.volumeLiters / Math.max(1e-9, state.baseVolume);
    return { frequency: thermalRatio / Math.max(.05, volumeRatio), impulse: thermalRatio };
  }
  function drawGasScene() {
    ...
    particles.forEach((particle) => {
      ...
      const speedRatio = speed / Math.max(1e-9, current.rmsSpeed);
      if (state.showTrails) line(context, box.left + particle.previousX * box.width, box.top + particle.previousY * box.height, x, y, `${color}${Math.round(clamp(92 + speedRatio * 58, 92, 180)).toString(16)}`, 1.2 + clamp((speedRatio - 1) * .8, 0, .8));
      context.fillStyle = color; context.beginPath(); context.arc(x, y, 2.7, 0, Math.PI * 2); context.fill();
      if (state.showVelocity && particles.indexOf(particle) % 4 === 0) { const length = Math.min(27, 5 + speedRatio * 14); const norm = Math.hypot(particle.gx, particle.gy) || 1; arrow(context, x, y, x + particle.gx / norm * length, y + particle.gy / norm * length, `${color}dd`, 1.2); }
    });
  ...
  function pistonHit(event) { const rect = refs.canvas.getBoundingClientRect(); const current = sample(); const box = containerGeometry(rect.width, rect.height, current); const pointerX = event.clientX - rect.left; const pointerY = event.clientY - rect.top; return Math.abs(pointerX - box.right) <= 22 && pointerY >= box.top - 16 && pointerY <= box.bottom + 16; }
  function setProgressFromPointer(event, offsetX = 0) { const rect = refs.canvas.getBoundingClientRect(); const current = sample(); const box = containerGeometry(rect.width, rect.height, current); const pointerX = event.clientX - rect.left + offsetX; if (state.mode === "microscopic") { const fraction = clamp((pointerX - box.left) / Math.max(1, box.maximumRight - box.left), 0, 1); state.baseVolume = Math.round((4 + fraction * 11) * 2) / 2; refs.volumeInput.value = state.baseVolume; } else state.progress = clamp((pointerX - box.left) / Math.max(1, box.maximumRight - box.left), 0, 1); state.running = false; renderUi(); }
  ...
  function processHandleHit(event) { const rect = refs.canvas.getBoundingClientRect(); const current = sample(); const box = containerGeometry(rect.width, rect.height, current); const pointerX = event.clientX - rect.left; const pointerY = event.clientY - rect.top; const handleX = box.left + (box.maximumRight - box.left) * state.progress; const handleY = rect.height - 38; return Math.hypot(pointerX - handleX, pointerY - handleY) <= 24; }
  function directHandleHit(event) { return state.mode === "microscopic" ? pistonHit(event) : processHandleHit(event); }
  let pistonDragOffset = 0;
  refs.canvas.addEventListener("pointerdown", (event) => { const rect = refs.canvas.getBoundingClientRect(); const current = sample(); const box = containerGeometry(rect.width, rect.height, current); if (!directHandleHit(event)) return; const pointerX = event.clientX - rect.left; const anchorX = state.mode === "microscopic" ? box.right : box.left + (box.maximumRight - box.left) * state.progress; pistonDragOffset = pointerX - anchorX; markCoreInteraction(); state.dragging = true; refs.canvas.classList.add("is-dragging"); refs.canvas.setPointerCapture(event.pointerId); setProgressFromPointer(event, -pistonDragOffset); }); refs.canvas.addEventListener("pointermove", (event) => { if (state.dragging) setProgressFromPointer(event, -pistonDragOffset); else refs.canvas.classList.toggle("is-hovering", directHandleHit(event)); }); refs.canvas.addEventListener("pointerup", (event) => { state.dragging = false; refs.canvas.classList.remove("is-dragging"); if (refs.canvas.hasPointerCapture(event.pointerId)) refs.canvas.releasePointerCapture(event.pointerId); }); refs.canvas.addEventListener("pointercancel", () => { state.dragging = false; refs.canvas.classList.remove("is-dragging"); }); window.addEventListener("resize", renderUi);
