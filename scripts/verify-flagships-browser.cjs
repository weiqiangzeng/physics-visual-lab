...
async function verifyLab(browser, lab, viewport) {
  ...
  try {
    ...
    if (lab.id === "force-composition") {
      ...
      const box = await canvas.boundingBox();
      await page.evaluate(() => window.forceCompositionLab.setState({ mode: "compose", composeControl: "free" }));
      ...
      await page.evaluate(() => window.forceCompositionLab.setState({ mode: "compose", composeControl: "angle", force1N: 6, force2N: 8, direction1Deg: 0, direction2Deg: 90 }));
      const angleBefore = await page.evaluate(() => window.forceCompositionLab.getState());
      const angleGeometry = await page.evaluate(() => window.forceCompositionLab.getInteractionGeometry());
      await page.mouse.move(box.x + angleGeometry.handles.includedAngle.x, box.y + angleGeometry.handles.includedAngle.y);
      await page.mouse.down();
      const anglePressed = await page.evaluate(() => window.forceCompositionLab.getState());
      await page.mouse.move(box.x + angleGeometry.handles.includedAngle.x + 46, box.y + angleGeometry.handles.includedAngle.y + 24);
      await page.mouse.up();
      const angleAfter = await page.evaluate(() => window.forceCompositionLab.getState());
      assert(anglePressed.force1N === angleBefore.force1N && anglePressed.force2N === angleBefore.force2N && angleAfter.force1N === angleBefore.force1N && angleAfter.force2N === angleBefore.force2N && Math.abs(angleAfter.direction2Deg - angleBefore.direction2Deg) > 5, `${lab.id}/${viewport.name}: fixed-force angle control violated its constraint`);
      await page.evaluate(() => window.forceCompositionLab.setState({ mode: "compose", composeControl: "magnitude", force1N: 6, force2N: 8, direction1Deg: 0, direction2Deg: 90 }));
      const magnitudeBefore = await page.evaluate(() => window.forceCompositionLab.getState());
      const magnitudeGeometry = await page.evaluate(() => window.forceCompositionLab.getInteractionGeometry());
      await page.mouse.move(box.x + magnitudeGeometry.handles.force1.x, box.y + magnitudeGeometry.handles.force1.y);
      await page.mouse.down();
      await page.mouse.move(box.x + magnitudeGeometry.handles.force1.x + 42, box.y + magnitudeGeometry.handles.force1.y);
      await page.mouse.up();
      const magnitudeAfter = await page.evaluate(() => window.forceCompositionLab.getState());
      assert(Math.abs(magnitudeAfter.force1N - magnitudeBefore.force1N) > 1 && magnitudeAfter.direction1Deg === magnitudeBefore.direction1Deg && magnitudeAfter.direction2Deg === magnitudeBefore.direction2Deg, `${lab.id}/${viewport.name}: fixed-angle magnitude control violated its constraint`);
      const scaleBefore = await page.evaluate(() => window.forceCompositionLab.getInteractionGeometry().scale);
      await page.evaluate(() => window.forceCompositionLab.setState({ force2N: 14 }));
      const scaleAfter = await page.evaluate(() => window.forceCompositionLab.getInteractionGeometry().scale);
      assert(Math.abs(scaleAfter - scaleBefore) < 1e-12, `${lab.id}/${viewport.name}: vector scale changed with force magnitude`);
      await page.evaluate(() => window.forceCompositionLab.setMode("multi"));
      const multiBefore = await page.evaluate(() => window.forceCompositionLab.getState());
      const multiGeometry = await page.evaluate(() => window.forceCompositionLab.getInteractionGeometry());
      await page.mouse.move(box.x + multiGeometry.handles.force3.x, box.y + multiGeometry.handles.force3.y);
      await page.mouse.down();
      await page.mouse.move(box.x + multiGeometry.handles.force3.x + 30, box.y + multiGeometry.handles.force3.y - 30);
      await page.mouse.up();
      const multiAfter = await page.evaluate(() => window.forceCompositionLab.getState());
      assert(Math.abs(multiAfter.force3N - multiBefore.force3N) > .5 && Math.abs(multiAfter.direction3Deg - multiBefore.direction3Deg) > 5, `${lab.id}/${viewport.name}: direct third-force drag did not update state`);
    ...
    if (lab.id === "resistivity") {
      ...
      await page.evaluate(() => window.resistivityLab.setMode("external"));
      ...
      const box = await canvas.boundingBox();
      const geometry = await page.evaluate(() => window.resistivityLab.getInteractionGeometry());
      const blankX = box.x + 16;
      const blankY = box.y + 16;
      await page.mouse.move(blankX, blankY);
      await page.mouse.down();
      await page.mouse.move(blankX + 120, blankY);
      await page.mouse.up();
      const afterBlankDrag = await page.evaluate(() => window.resistivityLab.getState().currentA);
      assert(closeTo(afterBlankDrag, beforeDrag, 1e-12), `${lab.id}/${viewport.name}: blank apparatus drag changed current`);
      const handleX = box.x + geometry.current.x;
      const handleY = box.y + geometry.current.y;
      await page.mouse.move(handleX, handleY);
      await page.mouse.down();
      const afterPress = await page.evaluate(() => window.resistivityLab.getState().currentA);
      assert(closeTo(afterPress, beforeDrag, 1e-12), `${lab.id}/${viewport.name}: current handle jumped on pointerdown`);
