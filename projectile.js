...
function setState(patch){if(patch.speed!==undefined)state.speed=clamp(patch.speed,5,35);if(patch.angle!==undefined)state.angle=clamp(patch.angle,5,85);if(patch.gravity!==undefined)state.gravity=clamp(patch.gravity,1.6,16);if(patch.target!==undefined)state.target=clamp(patch.target,2,45);if(patch.timeScale!==undefined)state.timeScale=clamp(patch.timeScale,.25,2);if(patch.time!==undefined)state.time=clamp(patch.time,0,calculate({...state,...patch}).flightTime);if(patch.running!==undefined)state.running=Boolean(patch.running);if(patch.guideStep!==undefined)state.guideStep=clamp(Math.round(patch.guideStep),0,guide.length-1);[["showComponents",refs.showComponentsToggle],["showAcceleration",refs.showAccelerationToggle],["showStrobe",refs.showStrobeToggle],["showCompare",refs.showCompareToggle]].forEach(([key,input])=>{if(patch[key]!==undefined)state[key]=Boolean(patch[key]);input.checked=state[key];});refs.speedInput.value=state.speed;refs.angleInput.value=state.angle;refs.gravityInput.value=state.gravity;refs.targetInput.value=state.target;refs.timeScaleInput.value=state.timeScale;render();}
